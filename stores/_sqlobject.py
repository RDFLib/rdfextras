...
from rdflib.term import Literal, URIRef, BNode, Statement
...
def _fromkey(key):
    if key.startswith("<") and key.endswith(">"):
        ...
        if m:
            ...
            if not lang:
                datatype = d["datatype"]
                return Literal(value, None, datatype)
            else:
                datatype = d["datatype"]
                return Literal(value, lang, None)
        else:
...
def _tokey(term):
    if isinstance(term, URIRef):
        ...
        if language and not datatype:
            language = language.encode("UTF-8")
            n3 = '"%s"@%s' % (value, language)
        elif datatype and not language:
            datatype = datatype.encode("UTF-8")
            n3 = '"%s"&<%s>' % (value, datatype)
        elif not datatype and not language:
            n3 = '"%s"' % (value)
        else:
            language = language.encode("UTF-8")
            n3 = '"%s"@%s' % (value, language)
...
class SQLObject(Store):
    ...
    def open(self, uri, create=True):
        if self.__open:
            return
        self.__open = True
        uri = self._uri if self._uri else uri
    ...
    def tokey(self, obj):
        if isinstance(obj, (URIRef, BNode)):
            ...
            return LITERAL, self._makeHash(_tokey(str(obj)))
        elif isinstance(obj, GraphValue):
            return URI, self._makeHash(_tokey(obj.identifier))
        elif obj is Any:
            return None, Any
    ...
    def insert(self, obj):
        if isinstance(obj, (URIRef, BNode)):
            ...
            return URI, self._insertURI(_tokey(obj.identifier))
        elif isinstance(obj, Statement):
            return LITERAL, self._insertLiteral(_tokey(str(obj)))
        raise ValueError, obj
