...
"""
    def delete(self, key=None):
        if key is None:
            key = self.key
        self.bucket.get(self._get_riak_key(key)).delete()

    def save(self):
        item = self.bucket.new(self._get_riak_key())
        item.set_data(data)
        item.store()
    ...
    def load(self):
        item = self.bucket.get(self._get_riak_key())
...
class Riak(Store):
    ...
    def make_transport(self):
        ...
        self.host = self.pb_host
        self.port = self.pb_port
        self.transport_class = RiakPbcTransport

        self.client_id = "\0\0\0\0"
        self.client = RiakClient(self.host, self.port,
                            transport_class = self.transport,
