"""
SQLAdBaseSQLObj.py
SQLAlchemy declarativeBase implementation based on SQLObject store.
"""
...
class SQLAlchemy(Store):
    ...
    def __init__(self, uri=None):
        ...
        # if not uri:
        #     print(dir(self))
        #     uri = self.engine.default_uri
        # self.open(uri=uri,create=False)
        # return None
        pass
    ...
    def _insertLiteral(self, value):
        ...
        lit = self._literals
        if not self.session.query(lit).filter(lit.hash == v_hash).count():
            new_lit = lit()
            new_lit.hash = v_hash
            new_lit.value = value
    ...
    def _insertURI(self, value=None, namespace=None, local_name=None):
        ...
        ns = self._ns
        if not self.session.query(ns).filter(
                        ns.hash == sql.bindparam('ns_hash')).count():
            new_ns = ns()
            new_ns.hash = ns_hash
            new_ns.value = namespace
            self.session.add(new_ns)
        rsrc = self._resources
        if not self.session.query(rsrc).filter(
                    rsrc.hash == sql.bindparam('rsrc_hash')).count():
            new_resource = rsrc()
            new_resource.hash = rsrc_hash
    ...
    def namespace(self, prefix):
        ...
        if not self.session.query(prefix_ns).filter(
                            prefix_ns.prefix == prefix).count():
            return None
    ...
    def bind(self, prefix, namespace):
        ...
        prefix = prefix.encode("utf-8")
        namespace = namespace.encode("utf-8")
        if not self.session.query(pns).filter(
                and_((pns.ns == namespace), (pns.prefix == prefix))).count():
            new_pns = pns()
