...
from rdfextras.sparql2sql import SPARQLError
from rdfextras.sparql2sql.sparqlGraph import SPARQLGraph
...
def _checkOptionals(pattern,optionals) :
    ...
    :type pattern: :class:`rdfextras.sparql.components.GraphPattern`
    :param optionals: graph pattern
    :type optionals: :class:`rdfextras.sparql.components.GraphPattern`
            ...
            if i > 0 :
                for j in xrange(0,i) :
                    if c in optionals[j].unbounds :
                        ...
                        # - the variable is in the previous optional: ie, 
                        #   Error!
                        raise SPARQLError(
    "%s is an illegal query string, it appears in a previous OPTIONAL clause" % c)

def _variablesToArray(variables,name='') :
    """Turn an array of Variables or query strings into an array of query 
    strings. If the 'variables' is in fact a single string or Variable, then 
    it is also put into an array.
    
    :param variables: a string, a unicode, or a Variable, or an array of 
    those (can be mixed, actually). As a special case, if the value is "*", 
    it returns None (this corresponds to the wildcard in SPARQL)
...
def _uniquefyList(seq, idfun=None):
    """
    Order preserving duplicate-discarding of list
    Ack: http://www.peterbe.com/plog/uniqifiers-benchmark
    """
    rt=[]
    seen = set()
    if idfun is None:
...
def _createInitialBindings(pattern) :
    """Creates an initial binding directory for the Graph Pattern by putting 
    a None as a value for each query variable.
...
class _SPARQLNode(object):
    ...
    :type clash: Boolean
    :ivar bound:  True or False depending on whether all variables are bound 
                  in self.binding
    :type bound: Boolean
    ...
    def __init__(self,parent,bindings,statements,tripleStore,expr=None) :
        ...
        :param bindings:   a dictionary with the bindings that are already 
                           done or with None value if no binding yet
        :param statements: array of statements from the 'where' clause. The 
        first element is for the current node, the rest for the children. If 
        empty, then no expansion occurs (ie, the node is a leaf)
        :param tripleStore: the 'owner' triple store
        :type tripleStore: :class:`rdfextras.sparql.components.sparqlGraph`
        """
        #If the parent is a successful OPTIONAL evaluation
        #(and thus marked as a proxy), then all its children
        #should be as well
        self.priorLeftJoin       = parent and parent.priorLeftJoin or False
        self.expr = expr
    ...
    def returnResult(self,select) :
        ...
        if len(self.children) > 0 :
            ...
            for c in self.children :
                ...
                # res is a list of dictionaries, so each tuple should be 
                # taken out and added to the result
                for t in res :
                    retval.append(t)
            return retval
        else :
                ...
                if not proxies:
                    prevBound=reduce(lambda x,y: x+y,
                               [list(_fetchBoundLeaves(o,previousBind=True))
                                                for o in self.optionalTrees],[])
                    if self.optionalTrees and prevBound:
                        pass
                    elif select :
                        for a in select :
                    ...
                    # Initial return block. If there is no optional 
                    # processing, that is the result, in fact, because the 
                    # for cycle below will not happen
                    if result:
                        retval = [result]
                else:
            ...
            if self.statement:
                ...
                for statement in [self.statement]+self.rest:
                    ...
                    (search_s,search_p,search_o) = searchTerms 
                                #(self._bind(s),self._bind(p),self._bind(o))
...
def query(graph, selection, patterns, optionalPatterns=[], initialBindings = {}):
    """
    A shorthand for the creation of a :class:`Query` instance, returning
    the result of a :func:`Query.select` right away. Good for most of
    the usage, when no more action (clustering, etc) is required.
    
    :selection: a list or tuple with the selection criteria,
    or a single string. Each entry is a string that begins with a"?".
    
    :patterns: either a ~rdfextras.sparql2sql.graphPattern.GraphPattern
    instance or a list of instances thereof. Each pattern in the
    list represent an 'OR' (or 'UNION') branch in SPARQL.
    
    :optionalPatterns: either a ~rdflib.sparql2sql.graphPattern.GraphPattern
    ...
    :return: list of query results as tuples
    """
...
def queryObject(graph, patterns, optionalPatterns=[], initialBindings = None) :
    """
    Creation of a :class:`Query` instance.
    
    :patterns: either a ~rdfextras.sparql2sql.graphPattern.GraphPattern
    instance or a list of instances thereof. Each pattern in the
    list represent an 'OR' (or 'UNION') branch in SPARQL.
    
    :optionalPatterns: either a ~rdfextras.sparql2sql.graphPattern.GraphPattern
    instance or a list of instances thereof. For each elements in
    the 'patterns' parameter is combined with each of the optional
    patterns and the results are concatenated. The list may be
    empty.
...
class Query :
    ...
    def ask(self) :
        ...
        :rtype: Boolean
        """
        if not [pat for pat in [self.top.statement]+self.top.rest
                    if not isGroundQuad(pat)] and \
                               self.top.bound and \
                           not self.top.clash and self.top.children:
            #A ground ASK or SELECT * query.  We interpret the existence of
            #children as an indication of success
            return True
        else:
    ...
    def clusterForward(self,selection) :
        ...
        :class:`rdfextras.sparql.graph.clusterForward`
        ...
        :return: a new triple store
        :rtype: :class:`rdfextras.sparql.components.SparqlGraph`
        """
        if self.parent1 != None and self.parent2 != None :
    ...
    def clusterBackward(self,selection) :
        ...
        seeds (when appropriate). It is based on the usage of the
        :class:`rdfextras.sparql.graph.clusterBackward`
        method for triple store.
...
# from rdfextras.sparql2sql.Query import EnoughAnswers
# from rdfextras.sparql2sql.Query import Query
# from rdfextras.sparql2sql.Query import isGroundQuad
# from rdfextras.sparql2sql.Query import query
# from rdfextras.sparql2sql.Query import queryObject
