...
from rdfextras.sparql2sql.sql.SqlBuilder import FLAG_STRAIGHT_JOIN
...
from rdfextras.tools.term_utils import term2Letter
...
def IsAVariable(term,bnode=False):
    """
    Returns true or false depending on whether or not this term is a variable
    (named or told BNodes are not considered variables)
    Can also be used to check that the term is an undistinguished BNode
    (useful since BNodeRef is a subclass of BNode and thus problematic for isinstance)
    """
    classesToCheck = BNode if bnode else (BNode,Variable)
    return isinstance(term, classesToCheck) and not isinstance(term,BNodeRef)
...
#        return self.outputMap[alias]
    ...
    def MatchTripleConditions(self, tableTuple, table, tp):
        ...
        if not IsAVariable(tp.sub):
            ...
            if isinstance(table,Table) and table.termEnumerations[SUBJECT]:
                cond.append("%s = '%s'"%(
                    self.TripleColumn(tableTuple, table, SUBJECT)[-1],
                    term2Letter(tp.sub)))
        if not IsAVariable(tp.pred):
            if table is self.GetTable('type'):
                ...
                if isinstance(table,Table) and table.termEnumerations[PREDICATE]:
                        cond.append("%s = '%s'"%(self.TripleColumn(tableTuple, table, PREDICATE)[-1],
                                                 term2Letter(tp.pred)))
        
        if not IsAVariable(tp.obj):
            ...
            if self.UseEvalOption(EVAL_OPTION_STRICT_DATATYPE) and \
               isinstance(tp.obj,Literal) and tp.obj.datatype:
                cond.append("%s = '%s' /*%s*/"%(self.TripleColumn(tableTuple, table,DATATYPE_INDEX)[0], 
                                                ...
                                                tp.obj.datatype))
            elif isinstance(table,Table) and isinstance(tp.obj,URIRef) and table.termEnumerations[OBJECT]:
                cond.append("%s = '%s'"%(self.TripleColumn(tableTuple, table, OBJECT)[-1],
                                         term2Letter(tp.obj)))
        if tp.context != None:
            if not IsAVariable(tp.context):
                ...
                if isinstance(table,Table) and table.termEnumerations[CONTEXT]:
                    cond.append("%s = '%s'"%(
                        self.TripleColumn(tableTuple, table, CONTEXT)[-1],
                        term2Letter(tp.context)))
            else:
            ...
            if len(self.rootBuilder.fromNamedList) > 0:
        ...
        for vname in varList:
            ...
            if (len(availableVars) == 1 or 
                ...
                if len(availableVars) > 1:
                    availableVars.sort(key=lambda x:int(x.tableTuple[2:]))
                v = availableVars[0]
                if outputVar:
    ...
    def AddStringJoin(self, var, outputName, addOutput=True):
        ...
        # figure out which table we need to join with
            ...
            if not needIdentifier and not needLiteral:
                raise Exception("Table not identified for strings for variable: " + var.name)
            ...
            if needIdentifier:
                ...
                joinConditions = ["%s.%s = %s"%(objTT1, idField, var.fieldName)]
                
                #We add the enumerated type of an arbitrary
                #variable sources that has an enumerated type column
                #as an extra join condition
                source = first([t for t in var.sources
                                    if isinstance(t.table,Table) and t.table.termEnumerations[t.pos]])
                if source:
                    joinConditions.append(
                        "%s.term_type = %s"%(
                            objTT1,
                            var.termFieldName))
                self.AddTupleJoin(objTT1, LEFT_OUTER_JOIN,' AND '.join(joinConditions))
    ...
    def CompareLiteralCondition(self, leftExp, leftLit, rightExp, rightLit, type, strictMatch):
        ...
        :param type: must be 'data_type' or 'language'
        :param strictMatch: True for strict adherence to RDF/SPARQL spec for match
          
        Literal expressions should be strings that will be processed here into appropriate hashes.
        
        :rtype: SQL WHERE condition string if needed, or empty string if no conditions are required. 
        ...
        if leftExp != rightExp:
            ...
            if not leftLit and not rightLit:
                ...
                if leftExp == None or rightExp == None:
                    ...
                    cond = "(%s = %s OR (coalesce(%s, %s) IS NULL))"%(leftExp,rightExp,leftExp,rightExp)
                else:
...
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import OPT_PUSH_DISTINCT
...
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import OPT_C2_SKIP_COALESCE
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import OPT_C3_SIMPLIFY_JOIN
...
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import EVAL_OPTION_ALLOW_BNODE_REF
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import DEFAULT_EVAL_OPTIONS
...
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import ViewTable    
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import TriplesTable
# from rdfextras.sparql2sql.sql.RdfSqlBuilder import AllObjectsTable
