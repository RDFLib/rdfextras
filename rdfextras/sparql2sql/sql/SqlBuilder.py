...
class FromJoinNode(object):
    ...
    def leafJoinTupleCandidate(self, tupleName):
        """
        Returns a generator over table tuples
        that are the leaves of this join tree as candidates
        for the source of the variables involved in a join condition 
        """
        if self.joinType in [CROSS_JOIN,INNER_JOIN,LEFT_OUTER_JOIN]:
            branches = self.joinType == LEFT_OUTER_JOIN and [self.left] or [self.left,self.right] 
            for branch in branches:
                if isinstance(branch,basestring):
                    if branch == tupleName:
                        yield branch
                else:
                    for leaf in branch.leafJoinTupleCandidate(tupleName):
                        yield leaf
        else:
            raise NotImplemented(repr(self))
        
    def getRepresentativeTuple(self):
        if self.joinType in [CROSS_JOIN,INNER_JOIN,LEFT_OUTER_JOIN]:
            return isinstance(self.left,basestring) and self.left or \
                   self.left.getRepresentativeTuple()
        elif self.joinType == LEFT_OUTER_JOIN:
            return isinstance(self.right,basestring) and self.right or \
                   self.right.getRepresentativeTuple()
        else:
            print self
            raise
...
#            sql += "TOP " + self.limit + " "
        ...
        if self.isAsk and self.rootBuilder == self:
            #If we are an ASK query then we can avoid an uneccessary top level 
            #SELECT EXISTS (..)
            return fromClause
        sql += " " + fromClause + " "
...
# Convenience
# from rdfextras.sparql2sql.sql.SqlBuilder import NONE_JOIN
...
# from rdfextras.sparql2sql.sql.SqlBuilder import RIGHT_OUTER_JOIN
# from rdfextras.sparql2sql.sql.SqlBuilder import FULL_OUTER_JOIN
# from rdfextras.sparql2sql.sql.SqlBuilder import CROSS_JOIN
# from rdfextras.sparql2sql.sql.SqlBuilder import JoinTypeMap
# from rdfextras.sparql2sql.sql.SqlBuilder import FLAG_STRAIGHT_JOIN
# from rdfextras.sparql2sql.sql.SqlBuilder import FLAG_NO_CACHE
