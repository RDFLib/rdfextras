...
class ParsedGroupGraphPattern(object):
    ...
    def __len__(self):
        return len([g for g in self.graphPatterns 
                    if isinstance(g,GraphPattern) \
                        and (g.triples or g.nonTripleGraphPattern) is not None])
...
#        super(ParsedOptionalGraphPattern,self).__init__(triples,graphPatterns)
    ...
    def __repr__(self):
        if self.graphPatterns is not None:
            return "OPTIONAL {%s %s}" % (
                    self.triples,self.graphPatterns)
        else:
...
class ParsedGraphGraphPattern(ParsedGroupGraphPattern):
    ...
    def __repr__(self):
        return "GRAPH %s { %s }" % (
            self.name,self.graphPatterns)

# Convenience
# from rdfextras.sparql2sql.bison.GraphPattern import BlockOfTriples
