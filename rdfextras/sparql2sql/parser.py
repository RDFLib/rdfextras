...
>>> query = parse('''PREFIX foaf: <http://xmlns.com/foaf/0.1/>
...
from rdfextras.sparql2sql import bison as components
from rdfextras.sparql import components as sparqlcomponents
...
def expand_string(result):
    """Unquote and expand the quoted string obtained as `result`."""
    s = result[0]
    if s.startswith('"""') or s.startswith("'''"):
        s = s[3:-3]
    else:
        s = s[1:-1]
    s = s.replace('\\t', '\t').replace('\\b', '\b').\
          replace('\\n', '\n').replace('\\r', '\r').\
          replace('\\f', '\f').replace('\\"', '"').\
          replace("\\'", "'").replace('\\\\', '\\')
    result[0] = s
...
Var = ((QM | USD) + VARNAME).setParseAction(
  refer_component(rdflib.Variable))
if DEBUG:
    Var.setName('Var')
...
# String:
#
# We don't use pyparsing's QuotedString here because of a bug whereby it
# erroneously removes backslashes even from those escape sequences it doesn't
# recognize (i.e. those that don't encode a quote, like \n or \t). So we just
# get the string out and then process the escapes manually.
# Note that the multiline quoting styles come first because otherwise "" and ''
# would be matched from their delimiters.
QString1 = Regex(r'\"\"\"(?:\\\"|\\\\|\"|\"\"|[^"])*\"\"\"', re.MULTILINE)
QString2 = Regex(r"\'\'\'(?:\\\'|\\\\|\'|\'\'|[^'])*\'\'\'", re.MULTILINE)
QString3 = Regex(r'\"(?:\\\"|\\\\|[^"])*\"')
QString4 = Regex(r"\'(?:\\\'|\\\\|[^'])*\'")
String = (QString1 | QString2 | QString3 | QString4).setParseAction(
          composition([expand_string,
                       refer_component(sparqlcomponents.ParsedString)]))
if DEBUG:
...
RDFLiteral = ((String + DOUBLE_HAT + IRIref).setParseAction(
  ...
  (String + Optional(LANGTAG, None)).setParseAction(
    refer_component(rdflib.Literal)))
if DEBUG:
...
INT = Regex(INT_re).setParseAction(composition(
  [refer_component(int), rdflib.Literal]))
...
FLOAT = Regex(FLOAT_re).setParseAction(composition(
  [refer_component(float), rdflib.Literal]))
...
codepoint_re = re.compile(r'(\\u[0-9a-fA-F]{4}|\\U[0-9a-fA-F]{8})')

def substitute_codepoint(match):
    return unichr(int(match.group(1)[2:], 16))

def preprocess(query_string):
    return codepoint_re.sub(substitute_codepoint, query_string)

def parse(stuff):
    stuff = preprocess(stuff)
    if DEBUG:
        tracer = ParseTracer()
