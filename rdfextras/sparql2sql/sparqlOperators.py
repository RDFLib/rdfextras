...
clause of a query. The parameters are either regular values or query 
strings. 
...
for checking whether ``?m`` is smaller than the (integer) value 42. It can 
be combined using the lambda function, for example:
...
from rdfextras.sparql2sql import _questChar
from rdfextras.sparql2sql import Debug
from graphPattern import _createResource

def queryString(v) :
    """
    Boolean test whether this is a a query string or not
    :param v: the value to be checked
    :rtype boolean: True if it is a query string
...
def getLiteralValue(v) :
    """
    Return the value in a literal, making on the fly conversion 
    on datatype (using the datatypes that are implemented)
    
    :param v: the Literal to be converted
    :rtype: the result of the conversion.
...
def getValue(param) :
    """
    Returns a *value retrieval function*. The return value can be plugged 
    in to a query; it would return the value of param directly if param is 
    a real value, and the run-time value if param is a query string of the type
    ``?xxx``. 
    ...
    :rtype: a function taking one parameter (the binding directory)
...
def isOnCollection(collection, item, triplets) :
    ...
    the 'item' is an element of the 'collection' (a.k.a. list). Both
    collection and item can be a real resource or a query string.
    ...
    :param collection: is either a query string (that has to be bound by the
        query) or an rdflib Resource` representing the collection
    :param item: is either a query string (that has to be bound by the query),
        an rdflib Resource or a data type value that is turned into a
        corresponding Literal (with possible datatype) that must be tested to be
        part of the collection
    :rtype: a function
    :returns: an adapted method
...
def generateCollectionConstraint(triples, collection, item):
    """
    ...
    :param triples: a :class:`sparqlGraph.SPARQLGraph` instance
    :param collection: is either a query string (that has to be bound by 
        the query) or an rdflib resource (ie, Literal, BNode, or 
        URIRef) representing the collection
    :param item: is either a query string (that has to be bound by the 
        query) or an rdflib resource (ie, Literal, BNode, or URIRef) 
        that must be tested to be part of the collection
    :rtype: a function suitable as a sparql filter
    :raises: :class:`~rdfextras.sparql.SPARQLError` : if the collection 
        or the item parameters are illegal (not valid resources for a 
...
def XSDCast(source, target=None):
    ...
    it simply creates a :class:`~rdflib.term.Literal` with the target datatype
    using the 'lexical' value of the source
    """
...
# Convenience
# from rdfextras.sparql2sql.sparqlOperators import queryString
# from rdfextras.sparql2sql.sparqlOperators import getLiteralValue
# from rdfextras.sparql2sql.sparqlOperators import getValue
# from rdfextras.sparql2sql.sparqlOperators import lt
# from rdfextras.sparql2sql.sparqlOperators import le
# from rdfextras.sparql2sql.sparqlOperators import gt
# from rdfextras.sparql2sql.sparqlOperators import ge
# from rdfextras.sparql2sql.sparqlOperators import eq
# from rdfextras.sparql2sql.sparqlOperators import neq
# from rdfextras.sparql2sql.sparqlOperators import bound
# from rdfextras.sparql2sql.sparqlOperators import isURI
# from rdfextras.sparql2sql.sparqlOperators import isIRI
# from rdfextras.sparql2sql.sparqlOperators import isBlank
# from rdfextras.sparql2sql.sparqlOperators import isLiteral
# from rdfextras.sparql2sql.sparqlOperators import str
# from rdfextras.sparql2sql.sparqlOperators import lang
# from rdfextras.sparql2sql.sparqlOperators import datatype
# from rdfextras.sparql2sql.sparqlOperators import isOnCollection
# from rdfextras.sparql2sql.sparqlOperators import generateCollectionConstraint
# from rdfextras.sparql2sql.sparqlOperators import addOperator
# from rdfextras.sparql2sql.sparqlOperators import XSDCast
# from rdfextras.sparql2sql.sparqlOperators import regex
# from rdfextras.sparql2sql.sparqlOperators import EBV
