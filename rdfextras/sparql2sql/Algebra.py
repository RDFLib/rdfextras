...
# -*- coding: utf-8 -*-
"""
An implementation of the W3C SPARQL Algebra on top of sparql-p's expansion
trees
...
We define eval(D(G), graph pattern) as the evaluation of a graph pattern with
respect to a dataset D having active graph G. The active graph is initially
the default graph.
"""
import unittest
...
from rdfextras.sparql2sql import DESCRIBE
from rdfextras.sparql2sql import SPARQLError
from rdfextras.sparql2sql import sparqlGraph
from rdfextras.sparql2sql import Query
from rdfextras.sparql2sql.Query import _variablesToArray
...
from rdfextras.sparql2sql.graphPattern import BasicGraphPattern
...
# from rdflib.store import Store
# from rdfextras.sparql2sql.bison.Query import AskQuery, SelectQuery, \
#     DescribeQuery, Query, Prolog
# from rdfextras.sparql2sql.bison.IRIRef import NamedGraph,RemoteGraph
# from rdfextras.sparql2sql.bison.SolutionModifier import ASCENDING_ORDER, \
#     ParsedOrderConditionExpression, DESCENDING_ORDER
# from rdfextras.sparql2sql import sparqlGraph, sparqlOperators, SPARQLError, \
#     Query, DESCRIBE
# from rdfextras.sparql2sql.bison.SPARQLEvaluate import unRollTripleItems, \
#     _variablesToArray
# from rdfextras.sparql2sql.bison.Util import ListRedirect
# from rdfextras.sparql2sql.bison.GraphPattern import *
# from rdfextras.sparql2sql.graphPattern import BasicGraphPattern
...
def LoadGraph(dtSet,dataSetBase,graph):
    ...
    if dataSetBase is not None:
        ...
        source = dtSet
    #GRDDL hook here!
    #print("CHECKING IF GRAPH IS ALREADY KNOWN", 
    #      repr(dtSet), 
    #      isinstance(graph.identifier, BNode), 
    #      dtSet in [g.identifier for g in graph.store.contexts()])
    if isinstance(graph.identifier, BNode) and \
                dtSet in [g.identifier for g in graph.store.contexts()]:
        graph += Graph(graph.store, dtSet)
        return
    try:
...
def TopEvaluate(
                query,dataset,passedBindings = None,DEBUG=False,exportTree=False,
                dataSetBase=None,
                extensionFunctions={}):
    """
...
#        print "---------------"
    ...
    if exportTree:
        # Allow this to raise an ImportError because the import is *bound* to
        # fail --- there is no record of any such file.
...
# Convenience
# from rdfextras.sparql2sql.Algebra import ImmutableDict
# from rdfextras.sparql2sql.Algebra import AlgebraExpression
# from rdfextras.sparql2sql.Algebra import EmptyGraphPatternExpression
# from rdfextras.sparql2sql.Algebra import NonSymmetricBinaryOperator
# from rdfextras.sparql2sql.Algebra import Join
# from rdfextras.sparql2sql.Algebra import LeftJoin
# from rdfextras.sparql2sql.Algebra import Union
# from rdfextras.sparql2sql.Algebra import GraphExpression
# from rdfextras.sparql2sql.Algebra import TestSPARQLAlgebra
# from rdfextras.sparql2sql.Algebra import TopEvaluate
# from rdfextras.sparql2sql.Algebra import MakeImmutableDict
# from rdfextras.sparql2sql.Algebra import ReduceGraphPattern
# from rdfextras.sparql2sql.Algebra import ReduceToAlgebra
# from rdfextras.sparql2sql.Algebra import RenderSPARQLAlgebra
# from rdfextras.sparql2sql.Algebra import LoadGraph
# from rdfextras.sparql2sql.Algebra import fetchUnionBranchesRoots
# from rdfextras.sparql2sql.Algebra import fetchChildren
# from rdfextras.sparql2sql.Algebra import walktree
# from rdfextras.sparql2sql.Algebra import print_tree
# from rdfextras.sparql2sql.Algebra import deepFlatten
