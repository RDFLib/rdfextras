...
class BerkeleyDB(Sleepycat):
    ...
    def _init_db_environment(self, homeDir, create=True):
        ...
        db_env.set_lk_max_locks(5000)
        db_env.set_lk_max_lockers(5000)
        db_env.set_lk_max_objects(5000)
    ...
    def begin_txn(self):
        ...
        try:
            if not thread.get_ident() in self.__dbTxn \
                                and self.is_open() and not self.__closing:
                ...
                txn = self.db_env.txn_begin(
                                self.__dbTxn[thread.get_ident()][0])
                self.__dbTxn[thread.get_ident()].append(txn)
        except Exception, e:
            print "begin_txn: ", e
            if txn != None:
        ...
        if thread.get_ident() in self.__dbTxn and self.is_open():
            try:
                ...
                if commit_root == True:
                    ...
                    #        self.__dbTxn[thread.get_ident()], 
                    #        txn, before, self.db_env.lock_stat()['nlocks']
