...
class AuditableStorage(Store):
    def __init__(self, storage):
        ...
        # NOTE: this store cannot be formula_aware as it does not have enough 
        # information to reverse the removal of a quoted statement
    ...
    def add(self, (subject, predicate, object_), context, quoted=False):
        ...
        context = context is not None and context.__class__(
                                    self.storage,context.identifier) or None
    ...
    def remove(self, (subject, predicate, object_), context=None):
        ...
        if None in [subject,predicate,object_,context]:
            if ctxId:
                ...
                for s,p,o,ctx in ConjunctiveGraph(self.storage).quads(
    ...
    def triples(self, (subject, predicate, object_), context=None):
        context = context is not None and context.__class__(
                                    self.storage,context.identifier) or None
        for (s,p,o),cg in self.storage.triples(
