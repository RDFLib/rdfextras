...
class AbstractSPARQLUnitTestCase(unittest.TestCase):
    ...
    def setUp(self):
        # WTF is a singleGraph when it's at home?
        if singleGraph:
...
class TestOPTVariableCorrelationTest(AbstractSPARQLUnitTestCase):
    ...
    def test_OPT_FILTER(self):
        rt=self.graph.query(BROKEN_OPTIONAL,
                                 processor='sparql2sql',
                                 DEBUG=True).result
        self.assertEqual(len(rt),1)
        self.failUnless(rt[0]==URIRef('http://example.org/interval3'),
...
class testSPARQLUnitTests(unittest.TestCase):
    ...
    from rdfextras.sparql2sql import Algebra
